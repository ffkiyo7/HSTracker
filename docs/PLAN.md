...
| **Phase 2** | 记牌器分区（牌库 / 手牌 / 已打出）+ 视觉重做 | 🚧 T1 分区合入（`1c44b212`）+ Bug T6 / T7 修账（`e9a67db6` / `7bd3192b`）；视觉重做 V1 矢量卡条 + 尺寸合入（`e3797ba8`，即 2.8 的改法 1~3 + 矢量化）；V2（段头 / 折叠 / 三行头 / 拖拽吸附）待写；2.6 / 2.7 未动。🎮 一局待打 |
...
### 本轮（2026-09-14）—— Phase 2 / T1 分区实战产出
...
| T6 | ~~根视图 `TrackerView` + `TrackerViewModel`，布局收口（见 1.4）~~ | ✅ 2026-09-11 卡点 ③ 实战通过。任务书 `docs/archive/tasks/phase1-t6-tracker-root-layout.md` |
...
`Player.swift:396` 的 `playerCardList` **保持原样不动** —— 它还被 `AppDelegate.swift:679`（套牌导出）依赖
（原来写的 `Game.swift:2161` 战绩上传那处在上游合并后已不存在，2026-09-13 核）。
...
替换为 Firestone 式三行头（**2026-09-04 定稿 D2**，比稿过程和参数见 `docs/archive/tasks/phase1-t5-tracker-header.md`「定稿」）：
...
> **状态（2026-09-15）**：下面「改法」的 1 / 2 / 3 和「主题贴图矢量化」已由 **Phase 2 / V1** 落地（`e3797ba8`，`docs/tasks/phase2-v1-vector-card-bars.md`）：
> 面板宽 = `min(窗口宽 × 8.9%, 窗口高 × 1.94% × 8.14)`，行高由宽推出，宽高比 8.14 : 1 由构造锁死，压缩时宽度同步收；`CardSize` 五档改成倍率；主题选择器撤掉。
> 改法 4（三行头统一到卡条行高）归 V2。本节其余内容是当初的分析，保留作依据。
...
任务书 `docs/archive/tasks/phase7-t1-session-recap-window.md`，与 T6 无文件交集，并行执行，卡点 ③ 那一局一起验。
