...
| 阶段 | 内容 | 状态 |
|---|---|---|
| **Phase 0** | 地基：驱动循环与窗口层 | 🟡 T0–T5 完成；**T6（埋点量程 + p50）本轮新增** |
...
| **Phase 6** | 排队时就显示牌组 | 🟡 T1 本轮开工；**「改动很小」的判断已被推翻**，见 Phase 6 一节 |
...
连同「D 段量程存疑」「B 段没埋点」两个旧的 🔴，一起进 `docs/tasks/phase0-t6-latency-spike.md`。
...
| Phase 6 排队显示 | 🎮 | 进队列 → 记牌器出现、30 张全在、计数框 `30 / 0`；匹配成功 → **不闪不重置**；退队列 → 消失；战棋 / 佣兵排队时**不出现** | 标准排一次。战棋侧**只需进队列再退出**，不用真打 |
...
| **T6** | **修埋点量程（D 段 + B 段 + E2E 归因），再按数据打 p50** ⬜ 本轮新增 | `LatencyProbe.swift` / `Game.swift` / `LogReader*.swift` |

**T6 的顺序是死的：口径没修好之前不许调参、不许优化。** 这个仓库已经在这上面栽过两次
（拿 Debug 数当 Release、拿只覆盖 1/20 的 D 段排序）。任务书
`docs/tasks/phase0-t6-latency-spike.md`。
...
| T3 | ETC / 下水道之王 改悬停浮出，备牌段整体消失（见 2.5，**从 Phase 2 提前**） | ✅ 已适配 3.6.7，未实战 |
