...
> ## 🔴 状态（2026-08-30 更新，接手前先读这段）
>
> **第 1 步「修口径」已完成**（D 段量程、B 段埋点、E2E 归因三件都做了）。
> **第 2 步「取 Release 基线」也已完成** —— 下面「这一轮的实测数据」那组 Debug 数
> **已作废，不要拿它做任何决策**。现行基线是：
>
> | 段 | p50 | p95 |
> |---|---|---|
...
> | **E2E** | **350.0** | **622.2** |
>
> Release 3.6.7.3733 / 5 局标准模式 / `dropped 0 outliers`。原始 dump 在
> `~/Desktop/dev/HSTracker-ab/logs/probe-2026-08-30-release-t6.txt`，
> 解读见 `docs/PROGRESS.md` 的「新基线读出来的三件事」。
>
> **剩下的是第 3 步：打 p50。目标是 D 段，不是 C 段** ——
> `Game.guiUpdateDebounce` 只有 16ms，最多解释 C 的 114.5 里的 16，剩下约 98ms 是
> `guiUpdateInFlight` 闸门在等上一轮刷新提交。**C 是 D 的因变量，压 D 会把 C 一起带下来**，
> 两段合计 196ms 是 E2E 350ms 里唯一能动的部分。A 段的 152ms 基本动不了（其中约 127ms
> 是炉石自己的写盘节奏）。

---

## 这一轮的实测数据（2026-08-30，卡点 ① · ⚠️ 已作废，见上面的状态段）
