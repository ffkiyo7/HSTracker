...
加一个 `Settings.useSwiftUITracker`。重写期间可随时切回旧 AppKit 记牌器做视觉对照和性能对比。**全部验收通过后删除该开关与旧代码**，不留长期分叉。

> **默认值：切片推进期间是 `false`，验收全通过后才翻成 true。** 这台机器每天在打游戏，
> 默认值不能是没在真实对局里看过的那条路径。2026-08-22 的 T2 已按这个默认落地。
