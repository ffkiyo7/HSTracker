...
| 构建 | Phase U3 合并后 Debug `clean build` `BUILD SUCCEEDED`；测试 **50 / 50 全绿**（2026-09-08，`xcodebuild test`，期望未改）；红龙 spike T1 加了 `RedDragonTests` 24 条，2026-09-11 **74 / 74 全绿**（已提交 `0a921d6b`）；Phase 2 / T1 + Bug T6 / T7 + V1 又加 32 条，2026-09-15 **106 / 106 全绿**（`7bd3192b`）；Bug T8 又加 6 条，**112 / 112 全绿**（未提交） |
| 阻塞 | 无。Codex 统一 review（09-15）的 3 条 P2 已由 **Bug T7** 修掉，第 1 条的残留（洗入副本被抽走少算 1）已由 **Bug T8** 根治（`docs/tasks/bug-t8-shuffled-copy-drawn-stays-in-deck.md`：解析时给实体落 `wasShuffledIntoDeck` 闩，动了 `TagChangeActions` / `Entity`），Claude + Codex review 通过，**112 / 112 全绿**，待提交、待 🎮 |
