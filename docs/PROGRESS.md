...
| **下一片** | **Phase 2 / V 视觉重做**（⑧⑨⑩ 合并：统一底色 + 矢量卡条 + 尺寸按比例 + 段头折叠，`docs/tasks/phase2-v-visual-redesign.md`）：🎨 **D2 已出**（用户选 B 酒馆 + A 方块费用格，低饱和、色盲可分），**等用户确认色值**，定稿后拆 V1 / V2 两本任务书，排在 Bug T6 之后。`card_size` 已切 small 应急。拖拽等用户确认上游「解锁窗口」机制够不够。之后：Phase 1 T8 动效 / Phase 4 / 4.3 其余 8 页设置。红龙 spike 已暂缓 |
