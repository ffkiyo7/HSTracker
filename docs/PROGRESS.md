...
| **下一片** | Bug T6 落地后派 **Phase 2 / T2 段头视觉**（`docs/tasks/phase2-t2-zone-headers.md`：Firestone 式透明段头 + 张数 + 折叠 + 长卡名）。⑨ 尺寸先用 `card_size = small` 试，不够再做 2.8。拖拽等用户确认上游「解锁窗口」机制够不够。之后：Phase 1 T8 动效 / Phase 4 / 4.3 其余 8 页设置。红龙 spike 已暂缓 |
