...
| **下一片** | **等用户定两件事**：⑧ 段头视觉往哪靠（A. 去掉 `DeckLens` 段头，改成与三行头同款 Belwe 字 + 细白格线的一行；B. 不要段头，段之间只留一条分隔线 + 段名小字；C. 你给参考图）；⑨ 尺寸（A. 先把 `card_size` 切到 medium / small 应急看看够不够；B. 直接做 2.8 尺寸重做，含贴图矢量化，是大片）。之后：Phase 1 T8 动效 / Phase 4 / 4.3 其余 8 页设置。红龙 spike 已暂缓，见「在做」 |
